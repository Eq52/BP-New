...
import { Trash2, Play, Film, Clock } from 'lucide-react';
...
export function HistoryPage({ onVideoClick }: HistoryPageProps) {
  ...
  return (
    <div className="h-full flex flex-col bg-[#0a0a0a]">
      {/* 头部 */}
      <header className={`flex items-center justify-between bg-[#0a0a0a] border-b border-white/5 ${
      ...
      {/* 历史列表 */}
      <div className={`flex-1 overflow-y-auto py-4 ${isDesktop ? 'px-8' : 'px-5'}`}>
        {history.length === 0 ? (
            ...
            {history.map((item) => (
              ...
              >
                ...
                <div 
                  ...
                  className="w-24 md:w-32 aspect-[3/4] flex-shrink-0 cursor-pointer relative group"
                >
                  <img
                    src={item.vod_pic}
                    alt={item.vod_name}
