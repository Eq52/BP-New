...
export function HomePage({ onVideoClick, onSettingsClick, onAddSourceClick, onSearchClick }: HomePageProps) {
  ...
  const loadVideos = useCallback(async (targetPage: number, reset: boolean = false) => {
    ...
    try {
      const response = await getVideoList(targetPage, isDesktop ? 24 : 18, currentCategory);
      ...
      setHasMore(response.list.length === (isDesktop ? 24 : 18));
    } catch (error) {
      console.error('加载影视失败:', error);
    } finally {
      setLoading(false);
    }
  }, [currentCategory, isDesktop]);
  ...
  return (
    <div className="h-full flex flex-col bg-[#0a0a0a]">
      ...
      {/* 桌面端标题栏 */}
      {isDesktop && (
        <header className="px-8 py-6 flex items-center justify-between bg-[#0a0a0a] border-b border-white/5">
          <div>
            <h1 className="text-white text-2xl font-bold">首页</h1>
            <p className="text-gray-500 text-sm mt-1">{currentSource?.name || '未选择源'}</p>
          </div>
        </header>
      )}

      {/* 搜索框 */}
      <div className={`px-5 py-2 ${isDesktop ? 'px-8 py-4' : ''}`}>
        <div 
      ...
      {/* 分类标签 */}
      <div className={`flex overflow-x-auto py-3 gap-2 scrollbar-hide ${isDesktop ? 'px-8' : 'px-5'}`}>
        {categories.map((cat) => (
          <button
            key={cat.id}
            onClick={() => {
      ...
      {/* 影视网格 */}
      <div 
        className={`flex-1 overflow-y-auto py-2 ${isDesktop ? 'px-8' : 'px-5'}`}
