...
export function DetailPage({ video, onBack, onPlay }: DetailPageProps) {
  ...
  return (
    <div className="h-full flex flex-col bg-[#0a0a0a]">
      {/* 头部 */}
      <header className={`flex items-center bg-[#0a0a0a] border-b border-white/5 ${
        isDesktop ? 'px-8 py-6' : 'px-5 py-4'
      }`}>
        <button 
          onClick={onBack}
      ...
      <div className="flex-1 overflow-y-auto">
        {loading ? (
          ...
          <div className={`py-5 ${isDesktop ? 'px-8' : 'px-5'}`}>
            {/* 封面和信息 */}
            <div className={`flex gap-6 ${isDesktop ? 'flex-row' : 'flex-col'}`}>
              {/* 封面 */}
              <div className={`rounded-xl overflow-hidden flex-shrink-0 shadow-xl shadow-purple-500/10 ${
                isDesktop ? 'w-48 xl:w-56' : 'w-32'
              } aspect-[3/4]`}>
                <img
              ...
              <div className="flex-1 flex flex-col justify-between">
                <div>
                  <h2 className="text-white text-xl font-bold mb-3 leading-tight">{displayData.vod_name}</h2>
                  
                  {displayData.vod_score && (
                    <div className="flex items-center mb-4">
                      <Star size={16} className="text-yellow-500 fill-yellow-500 mr-1.5" />
                      <span className="text-yellow-500 font-medium text-lg">{displayData.vod_score}</span>
                    </div>
                  )}
                  
                  <div className={`space-y-2 text-sm ${isDesktop ? 'grid grid-cols-2 gap-x-8 gap-y-2' : ''}`}>
                    ...
                    {displayData.vod_actor && (
                      <div className={`flex items-start text-gray-400 ${isDesktop ? 'col-span-2' : ''}`}>
                ...
                {/* 播放按钮 - 桌面端显示在信息区 */}
                {episodes.length > 0 && isDesktop && (
                  <div className="mt-6">
                    <button
                      onClick={() => handlePlay(0)}
                      className="bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 text-white font-medium py-3 px-8 rounded-xl flex items-center justify-center transition-opacity hover:opacity-90 shadow-lg shadow-purple-500/25"
                    >
            ...
            {displayData.vod_content && (
              <div className="mt-6">
                ...
                <p className={`text-gray-400 text-sm leading-relaxed ${isDesktop ? 'max-w-3xl' : ''}`}>
            ...
            {episodes.length > 0 && (
              <div className="mt-6 pt-4 border-t border-white/5">
                ...
                <div className={`grid gap-2 ${
                  isDesktop 
                    ? 'grid-cols-6 xl:grid-cols-8 2xl:grid-cols-10' 
                    : 'grid-cols-4'
                }`}>
