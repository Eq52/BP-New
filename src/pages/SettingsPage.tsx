...
import { ArrowLeft, Plus, Trash2, Save, Monitor, Database, Info, Check, AlertCircle, TestTube, Settings2 } from 'lucide-react';
    ...
    <div className="h-full flex flex-col bg-[#0a0a0a]">
      ...
      <header className={`flex items-center bg-[#0a0a0a] border-b border-white/5 ${
        ...
        <div className="flex items-center">
          <Settings2 className="w-5 h-5 text-purple-500 mr-2" />
          <h1 className="text-white text-lg font-bold">设置</h1>
        </div>
      </header>

      {/* 设置内容 */}
      <div className={`flex-1 overflow-y-auto pb-24 ${isDesktop ? 'px-8 py-6' : 'px-5 py-4'}`}>
        <div className={`grid gap-6 ${isDesktop ? 'grid-cols-2' : 'grid-cols-1'}`}>
          ...
          <section className="mb-8">
            ...
            <div className="bg-[#141414] border border-white/5 rounded-xl p-4 space-y-3">
              {sources.length === 0 ? (
                <p className="text-gray-500 text-sm text-center py-4">暂无影视源</p>
              ) : (
                sources.map((source) => (
                  <div
                    key={source.id}
          ...
          <section className="mb-8">
            ...
            <div className="bg-[#141414] border border-white/5 rounded-xl p-4 space-y-3">
              <div className="flex justify-between">
                ...
                <span className="text-white text-sm">V3.0</span>
              </div>
              <div className="flex justify-between">
